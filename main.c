...
int main(void) {
	...
	pad_white, pad_start, pad_select, pad_home, pad_l3, pad_r3, pad_l, pad_r, pad_left_analog_x,
	pad_left_analog_y, pad_right_analog_x, pad_right_analog_y;
	
	bool mode_DPAD, mode_Lanalog, mode_Ranalog;
	mode_DPAD=true;
	mode_Lanalog=false;
	mode_Ranalog=false;
	...
	for (;;) {
		...
		pad_up = !bit_check(PIND, 3);
		pad_down = !bit_check(PIND, 2);
		pad_left = !bit_check(PIND, 1);
		pad_right = !bit_check(PIND, 0);
		pad_x = !bit_check(PIND, 4); //square
		pad_y = !bit_check(PINC, 6); //triangle 
		pad_a = !bit_check(PIND, 7); //x 
		pad_b = !bit_check(PINE, 6); //circle
		pad_black =  !bit_check(PINB, 4); //lb
		pad_white =  !bit_check(PINB, 5); //rb
		pad_l = !bit_check(PINB, 6); //lt 
		pad_r = !bit_check(PINB, 2); //rt
		pad_start =  !bit_check(PINB, 3);
		pad_select =  !bit_check(PINB, 1);
		pad_home = !bit_check(PINF, 7);
		pad_l3 =  !bit_check(PINF, 6);
		pad_r3 =  !bit_check(PINF, 5);

		pad_left_analog_x = pad_left_analog_y = pad_right_analog_x = pad_right_analog_y = 0x7F;

	if(mode_Lanalog){
		if(!bit_check(PIND, 1)) {
			pad_left_analog_x = 0x00; //left Analog left
		} else if(!bit_check(PIND, 0)) {
			pad_left_analog_x = 0xFF; //left Analog right
		}

		if(!bit_check(PIND, 3)) {
			pad_left_analog_y = 0x00; //left Analog Up
		} else if(!bit_check(PIND, 2)) {
			pad_left_analog_y = 0xFF; //left Analog Down
		}
	}
